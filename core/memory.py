...
class MemoryEngine:
    def __init__(self, db_path=None):
        self.db_path = Path(MEMORY_DB if db_path is None else db_path)
    ...
    def _init_db(self):
        with self._connect(self.db_path) as conn:
            conn.execute(
                ...
                "user_input TEXT, system_response TEXT, "
                "timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, metadata JSON)"
            )
            conn.execute(
                "CREATE TABLE IF NOT EXISTS user_profile ("
                "id INTEGER PRIMARY KEY, preferences JSON, projects JSON, "
                "recurring_goals JSON, last_updated DATETIME DEFAULT CURRENT_TIMESTAMP)"
            )
    ...
    def get_conversation_history(self, session_id, limit=10):
        with self._connect(self.db_path) as conn:
            rows = conn.execute(
                "SELECT user_input,system_response,timestamp FROM conversations "
    ...
    def _ensure_provenance_table(self):
        with self._connect(self.provenance_db) as conn:
            conn.execute(
                """CREATE TABLE IF NOT EXISTS provenance_memory (
    ...
    def store_with_provenance(self, content, source, confidence=0.5, tags=None, status="KNOWN"):
        """status: KNOWN | INFERRED | NOT_VERIFIED | EXPERIMENTAL"""
        self._ensure_provenance_table()
        with self._connect(self.provenance_db) as conn:
            conn.execute(
                "INSERT INTO provenance_memory(content,source,confidence,tags,status) VALUES(?,?,?,?,?)",
                (content, source, float(confidence),
                 json.dumps(tags or [], ensure_ascii=False), status),
            )
    ...
    def search_with_provenance(self, query, limit=10):
        ...
        with self._connect(self.provenance_db) as conn:
            rows = conn.execute(
                "SELECT content,source,confidence,status,created_at FROM provenance_memory "
        ...
        return [
            {"content": r[0], "source": r[1], "confidence": r[2], "status": r[3], "time": r[4]}
            for r in rows
        ]
