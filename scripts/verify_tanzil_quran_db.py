...
def db_rows(path: str) -> list[tuple[int, int, str]]:
    with sqlite3.connect(path) as con:
        total = con.execute(
            "SELECT COUNT(*) FROM knowledge_fts WHERE source='قرآن' AND category='معنوی'"
        ).fetchone()[0]
        rows = con.execute(
    ...
    if total != EXPECTED_AYAHS:
        raise ValueError(
            f"DB has {total} قرآن/معنوی rows; expected exactly {EXPECTED_AYAHS}"
        )

    result = []
    for index, (content,) in enumerate(rows, 1):
        if not isinstance(content, str):
            raise ValueError(f"Invalid DB row {index}: non-text content")
        marker = " | "
        if marker not in content:
            raise ValueError(f"Invalid DB row {index}: missing ' | ' separator")
        prefix, translation = content.split(marker, 1)
        words = prefix.split()
        if len(words) != 4 or words[0] != "سوره" or words[2] != "آیه":
            raise ValueError(f"Invalid DB row {index}: {content!r}")
