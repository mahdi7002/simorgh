"""
persona_chat.py — موتور گفت‌وگوی زنده‌ی ۱۲ پرسونا (نسخه‌ی سبک و مستقل)
====================================================
...
چرا این فایل از core/chat.py جداست (عمدی، نه بدهیِ فنی):
core/chat.py نسخه‌ی کاملِ پرسوناهاست — با ابزارهای جست‌وجوی قرآن/شعر/کتاب،
منطقِ handoff، و تشخیصِ سطحِ مدل — اما به همین دلیل به کل استکِ سیمرغ
وابسته است. این فایل عمداً سبک نگه داشته شده تا حتی بدونِ نصبِ FastAPI یا
وابستگی‌های سنگین، فقط با Python استاندارد اجرا بشه. هردو فایل زبانِ
منشور (اصل صفر، منعِ خودبهبودی) رو دارن؛ فقط عمقِ ابزارهاشون فرق می‌کنه.
